...
public class cameraRotation : MonoBehaviour 
{
	...
	void Start () 
    {
        ...
        if(followTarget)
        {
            ...
            tempquaternion = Quaternion.Euler(0, followTarget.rotation.eulerAngles.y, 0);
	...
	void Update ()
    {
        ...
        {
            istrans = true;
            float a = Input.GetAxis("Mouse X") * rotateSpeed;
            float b = Input.GetAxis("Mouse Y") * rotateSpeed;
            h += a;
            v += b;
            followTarget.rotation = Quaternion.Euler(-v, h, 0);
            //  followTarget.Rotate(-v, h, 0, Space.Self);
        }
        if (Input.GetAxis("Mouse ScrollWheel") != 0)
        {
            this.GetComponent<Camera>().fieldOfView -= Input.GetAxis("Mouse ScrollWheel") * 10f;
        }
#endif
        //当没有触控时就返回
    ...
    public void cameraRecover()
    {
        ...
        followTarget.rotation = Quaternion.Slerp(followTarget.rotation, tempquaternion, 1f);
        transform.position = cameraPosition;
        
#if UNITY_EDITOR
        h = this.transform.rotation.eulerAngles.x;
        v = this.transform.rotation.eulerAngles.y;
